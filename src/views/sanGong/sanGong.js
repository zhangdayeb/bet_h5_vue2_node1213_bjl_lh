...
import sanGongService from '@/service/sanGongService.js'
import userService from '@/service/userService.js'
import {THREE_URL, SocketTask} from '@/utils/socket'
...
//下注矩形背景颜色
const blockBackgroundColor = '#0046C3'
export default {
    name: 'SanGong',
    components: {
        SelectChip,
	...
	data(){
		return {
            ...
            tableInfoTimer: null,
			canvasApi: null,
            welcomeMsg: this.$t('sanGong.welcome'),
            //初始化音频
            ...
            betLocationList:[
                ...
                {x: 0,y: 68, chips: [], betAmount: 0, showChips: [], id: 46, onlyType: '', name: '三公超牛'},
                {x: 0, y: 68, chips: [], betAmount: 0, showChips: [], id: 47, onlyType: '', name: '三公超牛'},
                {x: 0, y: 68, chips: [], betAmount: 0, showChips: [], id: 48, onlyType: '', name: '三公超牛'},
                {x: 0, y: 135, chips: [], betAmount: 0, showChips: [], id: 40, onlyType: '', name: '三公翻倍'},
                {x: 0, y: 135, chips: [], betAmount: 0, showChips: [], id: 42, onlyType: '', name: '三公翻倍'},
                {x: 0, y: 135, chips: [], betAmount: 0, showChips: [], id: 44, onlyType: '', name: '三公翻倍'},
                {x: 0, y: 203, chips: [], betAmount: 0, showChips: [], id: 41, onlyType: '', name: '三公平倍'},
                {x: 0, y: 203, chips: [], betAmount: 0, showChips: [], id: 43, onlyType: '', name: '三公平倍'},
                {x: 0, y: 203, chips: [], betAmount: 0, showChips: [], id: 45, onlyType: '', name: '三公平倍'},
            ],
            // 重复项目
            repeatData: [],
            //需要取消的下注数据
    ...
    //             let time = 2000
				// setTimeout(() => {
    //                 this.setResultSound(this.resultInfo.result['1'], 'banker')
				// },time - 1000)
				// setTimeout(() => {
    //                 this.setResultSound(this.resultInfo.result['2'], 'player1')
				// },time + 2700)
    //             setTimeout(() => {
    //                 this.setResultSound(this.resultInfo.result['3'], 'player2')
				// },time + 5000)
    //             setTimeout(() => {
    //                 this.setResultSound(this.resultInfo.result['4'], 'player3')
				// },time + 7500)
			}
		},
         ...
         * 处理结果声音
         * @param result 结果信息
         * @param name banker  player1 player2 player3
        */
        setResultSound(result, name) {
            let time = 1200
            this.audioHandle.startSoundEffect(`${name}.wav`)
            switch(result.result_num.g) {
                case 0: 
                    setTimeout(() => {
                        this.audioHandle.startSoundEffect(`point${result.result_num.num}.wav`)
                    }, time)
                break
                case 1:
                    setTimeout(() => {
                        this.audioHandle.startSoundEffect(`singleMan${result.result_num.num}.wav`)
                    }, time)
                break
                case 2:
                    setTimeout(() => {
                        this.audioHandle.startSoundEffect(`doubleMan${result.result_num.num}.wav`)
                    }, time)
                break
                case 3:
                    setTimeout(() => {
                        this.audioHandle.startSoundEffect(`tripleMan.wav`)
                    }, time)
                break
            ...
            if(result.result_num.num >= 20) {
                switch(result.result_num.num.toString()) {
                    case '33':
                        setTimeout(() => {
                            this.audioHandle.startSoundEffect(`threeK.wav`)
                        }, time)
                    break
                    case '32':
                        setTimeout(() => {
                            this.audioHandle.startSoundEffect(`threeQ.wav`)
                        }, time)
                    break
                    case '31':
                        setTimeout(() => {
                            this.audioHandle.startSoundEffect(`threeJ.wav`)
                        }, time)
                    break
                    case '30':
                        setTimeout(() => {
                            this.audioHandle.startSoundEffect(`three10.wav`)
                        }, time)
                    break
                    case '21':
                        setTimeout(() => {
                            this.audioHandle.startSoundEffect(`threeA.wav`)
                        }, time)
                    break
                    case '23':
                    case '999':
                        setTimeout(() => {
                            this.audioHandle.startSoundEffect(`three3.wav`)
                        }, time)
                    break
                    default:
                        setTimeout(() => {
                            this.audioHandle.startSoundEffect(`three${result.result_num.pai[0]}.wav`)
                        }, time)
                    break
                }
            }
        ...
        init(){
            ...
            let wrapper = document.getElementById("san-wrapper");
        ...
        */
        ...
        getTableInfo() {
            clearTimeout(this.tableInfoTimer)
            sanGongService.tableInfo({id: this.tableId}).then(res => {
                this.tableInfo = res
        ...
        */
        getCurrentRecord(){
            sanGongService.getBetCurrentRecord({id: this.tableId}).then(res => {
        ...
        */
        handleSubmit(){
            ...
            sanGongService.betOrderService(data).then(res => {
                this.initCancelData()
        ...
        */
        drawTable() {
            ...
            this.canvasApi.drawText(this.$t("sanGong.player1"), firstRowRectWidth / 2 - 10, xianFontY, fontSize, 'txt-xian1', color, '', true)
            this.canvasApi.drawText(this.$t("sanGong.player2"), secondRectX + firstRowRectWidth / 2 - 10, xianFontY, fontSize, 'txt-xian2', color, '', true)
            this.canvasApi.drawText(this.$t("sanGong.player3"), thirdRectX +  firstRowRectWidth / 2 - 10, xianFontY, fontSize, 'txt-xian2', color, '', true)
            //翻倍矩形y轴坐标
            let row2RectY = 22
            //下注矩形高度
            let blockHeight = (this.canvasHeight - 28) / 3
            ...
            let betChipWidth = 5
            //第二行 超级三公
            const superGong = 'super-gong'
            this.betLocationList[0].x = firstRowRectWidth / 2 - betChipWidth
            this.betLocationList[0].onlyType = `rect-${allowBetName}-0-${superGong}0`
            ...
            this.betLocationList[1].onlyType = `rect-${allowBetName}-1-${superGong}1`
            ...
            this.betLocationList[2].onlyType = `rect-${allowBetName}-2-${superGong}2`
            this.betLocationList[2].y = row2RectY
            this.canvasApi.drawRect(thirdRectX, row2RectY, firstRowRectWidth, blockHeight, blockBorder, `rect-${allowBetName}-2-${superGong}2`, blockBackgroundColor,'' ,  rectRadius,  rectRadius)
            //第三行的翻倍矩形y轴坐标
            ...
            // 翻倍字体宽度
            let doubleFontWidth = 40
            // 比率字体宽度
            let ratioFontWidth = 25
            //超牛字体在方块中y轴坐标中心点
            let doubleFontY = this.betLocationList[0].y + (blockHeight / 2)
            /***画文字的，最好不要删除掉，后面找一个为什么fabric为什么在ios中fontsize不起作用，不至于这样啊，好坑***/
            //赔率
            let odds = `1:0.95`
            this.canvasApi.drawText(this.$t("sanGong.superCalf"),firstRowRectWidth / 2 - doubleFontWidth, doubleFontY-20, doubleFontSize, `txt-${allowBetName}-0-${superGong}0`, color, 'Helvetica', true)
            this.canvasApi.drawText(odds,firstRowRectWidth / 2 - ratioFontWidth-5, doubleFontY, fontSize, `txt-${allowBetName}-0-ratio0`, color, 'Helvetica', true)
            this.canvasApi.drawText(this.$t("sanGong.superCalf"),secondRectX + firstRowRectWidth / 2 - doubleFontWidth, doubleFontY-20, doubleFontSize, `txt-${allowBetName}-1-${superGong}1`, color, 'Helvetica', true)
            this.canvasApi.drawText(odds,secondRectX + firstRowRectWidth / 2 - ratioFontWidth-5, doubleFontY, fontSize, `txt-${allowBetName}-1-ratio1`, color, 'Helvetica', true)
            this.canvasApi.drawText(this.$t("sanGong.superCalf"),thirdRectX + firstRowRectWidth / 2 - doubleFontWidth, doubleFontY-20, doubleFontSize, `txt-${allowBetName}-2-${superGong}2`, color, '', true)
            this.canvasApi.drawText(odds,thirdRectX + firstRowRectWidth / 2 - ratioFontWidth-5, doubleFontY, fontSize, `txt-${allowBetName}-2ratio2`, color, '', true)
            //第二行翻倍字体y轴坐标
            let double2FontY = this.betLocationList[4].y + (blockHeight / 2)
            this.canvasApi.drawText(this.$t("sanGong.double"),firstRowRectWidth / 2 - 18, double2FontY-20, doubleFontSize, `txt-${allowBetName}-3-${doubleName}3`, color, '', true)
            this.canvasApi.drawText(odds,firstRowRectWidth / 2 - ratioFontWidth-5, double2FontY, fontSize, `txt-${allowBetName}-3-ratio3`, color, '', true)
            this.canvasApi.drawText(this.$t("sanGong.double"),secondRectX + firstRowRectWidth / 2 - 18, double2FontY-20, doubleFontSize, `txt-${allowBetName}-4-${doubleName}4`, color, '', true)
            this.canvasApi.drawText(odds,secondRectX + firstRowRectWidth / 2 - ratioFontWidth-5, double2FontY, fontSize, `txt-${allowBetName}-4-ratio4`, color, '', true)
            this.canvasApi.drawText(this.$t("sanGong.double"),thirdRectX + firstRowRectWidth / 2 - 18, double2FontY-20, doubleFontSize, `txt-${allowBetName}-5-${doubleName}5`, color, '', true)
            this.canvasApi.drawText(odds,thirdRectX + firstRowRectWidth / 2 - ratioFontWidth-5, double2FontY, fontSize, `txt-${allowBetName}-5-ratio5`, color, '', true)
            //平倍字体y轴坐标
            let double3FontY = this.betLocationList[7].y + (blockHeight / 2) 
            this.canvasApi.drawText(this.$t("sanGong.ordinary"),firstRowRectWidth / 2 - 20, double3FontY-20, doubleFontSize, `txt-${allowBetName}-6-${doubleName}6`, color, '', true)
            this.canvasApi.drawText('1:1',firstRowRectWidth / 2 - 15, double3FontY, fontSize, `txt-${allowBetName}-6-ratio6`, color, '', true)
            this.canvasApi.drawText(this.$t("sanGong.ordinary"),secondRectX + firstRowRectWidth / 2 - 20, double3FontY-20, doubleFontSize, `txt-${allowBetName}-7-${doubleName}7`, color, '', true)
            this.canvasApi.drawText('1:1',secondRectX + firstRowRectWidth / 2 - 15, double3FontY, fontSize, `txt-${allowBetName}-7-ratio7`, color, '', true)
            this.canvasApi.drawText(this.$t("sanGong.ordinary"),thirdRectX + firstRowRectWidth / 2 - 20, double3FontY-20, doubleFontSize, `txt-${allowBetName}-8-${doubleName}8`, color, '', true)
            this.canvasApi.drawText('1:1',thirdRectX + firstRowRectWidth / 2 - 15, double3FontY, fontSize, `txt-${allowBetName}-8-ratio8`, color, '', true)
        },
